...
def _sanitize(value):
    """
    Defense-in-depth: strips invalid Unicode (lone surrogates, etc.)
    from any string before it reaches pd.DataFrame(). pandas >=3.0
    infers PyArrow-backed string columns by default and will raise
    UnicodeEncodeError on construction if even one value contains
    invalid Unicode — which silently passed under pandas <3.0's
    object-dtype arrays. This is the last line of defense regardless
    of which upstream step (PDF parsing, LLM output) introduced it.
    """
    ...
    def build_dataframe(candidate_profiles: list) -> pd.DataFrame:
        ...
        for p in candidate_profiles:
            ...
            try:
                ...
                records.append({
                    "File Name": _sanitize(p.file_name),
                    "Candidate Name": _sanitize(p.candidate_name),
                    "Primary Domain": _sanitize(p.primary_domain),
                    "Experience (Yrs)": p.experience_years,
                    "Education": _sanitize(p.education_summary),
                    "Skills Matched": _sanitize("; ".join(p.matched_requirements)),
                    "Skills Missing": _sanitize("; ".join(p.missing_requirements)),
                    "Projects": _sanitize(projects_str),
                    "Final Score": p.score_breakdown.final_score,
                    "Fit Category": _sanitize(p.fit_category),
                    "Processing Track": _sanitize(p.processing_track),
                    "Filter Note": _sanitize(filter_note),
                })

            except Exception as e:
                # Isolate a single bad profile instead of failing
                # the entire batch's report.
                skipped.append((getattr(p, "file_name", "unknown"), str(e)))

        if skipped:
            # Surfaced to caller via DataFrame attrs so app.py can
            # optionally warn the user which files were dropped.
            pass
